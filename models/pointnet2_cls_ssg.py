...
def placeholder_inputs(batch_size, num_point, withnormal):
    pointclouds_pl = tf.placeholder(tf.float32, shape=(batch_size, num_point, 3+withnormal*3))
...
def get_model_single_scale(point_cloud, is_training, bn_decay=None, keep_prob=0.5):
    ...
    # Note: When using NCHW for layer 2, we see increased GPU memory usage (in TF1.4).
    # So we only use NCHW for layer 1 until this issue can be resolved.
    l1_xyz, l1_points, l1_indices = pointnet_sa_module(l0_xyz, l0_points, npoint=None, radius=None, nsample=None, mlp=[64,64,128,128,256,256,512,1024], mlp2=None,
                                                       group_all=True, is_training=is_training, bn_decay=bn_decay, scope='layer1')
